...
> 该指南将帮助您安装 寿司峰 系列整合包，并调整好必要的设置。它对于绝大多数 寿司峰 整合包都是通用的。

## 安装整合包

首先访问寿司峰玩家 **QQ 群：`1045084460`**

在群文件中，您可以找到整合包文件夹。在文件夹中找到您想要游玩的服务器整合包，并**下载最新版本**。
...
!!! warning "一些注意事项"
    1. 记得把上述JVM参数设置在整合包版本设置里。如果您把它填进全局设置里，您就玩不了除 寿司峰 以外的其他任何Minecraft版本了。
    2. 对于 Windows 操作系统，整合包安装的路径中**只能包含英文字符**。也就是说，您的 .minecraft 文件夹的绝对路径**不能包含中文**。如果必须包含（比如您的用户名包含中文），请参见下方[相关服务](guide-join.md#update-service)章节。

## 选择登录方式

为了安全和兼容性，寿司峰 仅支持 [LittleSkin](https://littleskin.cn) 登录。
...
> 您可以在这里找到 寿司峰 面向玩家开放的一些相关服务。

### 寿司峰更新服务 {#update-service}

如果您按照 [进服指南](#update-setup) 设置了 JVM 参数，您的客户端会在游戏启动时**自动检查更新**。这会保证您的客户端始终与最新版整合包同步，您不需要在每次更新时重新安装整合包。
...
您可以访问 网页地图 查看各服务器的实时地图。具体请见各整合包页面。
